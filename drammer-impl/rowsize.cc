...
// Snapdragon 820
   {"Pixel XL",  "marlin",           "msm8996",       "msm8996", 22,  0,      "Pixel XL"},
...
int RS_autodetect(void) {
    ...
    print("[RS] Allocating 512 ion chunk\n");
    struct ion_data data;
    data.handle = ION_alloc(K(512));
    if (data.handle == 0) {
        perror("Could not allocate 512K chunk for row size detection");
        exit(EXIT_FAILURE);
    }
    data.len = K(512);
    ION_mmap(&data);
